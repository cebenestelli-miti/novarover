"""Grazing Obstacle Test: RViz + mock base.

Robot starts at (0,0), waypoint at (12,0).
A small obstacle slightly intrudes into the direct path so the mower only
needs a modest deviation to avoid clipping it.

Scenario (approximate):
- waypoint line: from (0, 0) to (12, 0)
- obstacle: ~0.8 m cube, modeled as circle radius 0.4 m
- obstacle center: (7.5, 0.40)

Goals:
- mower approaches in GO_TO_GOAL
- may briefly enter FOLLOW_OBSTACLE / BUG behavior
- avoids clipping the obstacle without a huge detour
- does not enter BLOCKED_STOP or dead-end rejection
- returns to GO_TO_GOAL and completes the mission deterministically
"""
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            # Mock base: small grazing obstacle slightly above the path.
            # 0.8 m cube approximated as circle radius 0.4 m at (7.5, 0.40).
            Node(
                package="mower_base",
                ...
                parameters=[
                    {
                        ...
                        "virtual_obstacle_center_x": 7.5,
                        "virtual_obstacle_center_y": 0.40,
                        "virtual_obstacle_radius_m": 0.4,
