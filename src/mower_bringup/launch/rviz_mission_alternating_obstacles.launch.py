"""Alternating Obstacles Test: RViz + mock base.

Robot starts at (0,0), waypoint at (14,0).
Four ~1 m cube obstacles are placed alternately above and below the path:
...
Each cube is approximated as a circle of radius 0.5 m.

Goals:
- verify multiple sequential avoidance cycles:
  GO_TO_GOAL → FOLLOW_OBSTACLE → GO_TO_GOAL → FOLLOW_OBSTACLE → GO_TO_GOAL ...
- avoid oscillation, spirals, or drift away from the path
- avoid BLOCKED_STOP
- ensure forward progress after each obstacle and deterministic mission completion
"""
...
def generate_launch_description():
    ...
    # Use the 12 m straight-line mission; final waypoint at (12,0) still exercises
    # all four obstacles placed between x≈4 and x≈10.
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                description="Path to .waypoints mission file (straight line).",
            ),
            DeclareLaunchArgument(
                "farm_config",
            ...
            # Mock base: four alternating obstacles, each ~1 m cube (radius 0.5 m).
            # The mock_base_interface_node supports up to three virtual obstacles; we
            # approximate the four-cube pattern with three circles that preserve the
            # alternating geometry along the path:
            #   - (4.0,  0.6) r=0.5
            #   - (7.0, -0.6) r=0.5 (represents the pair near x=6 and x=8)
            #   - (10.0, -0.6) r=0.5
            Node(
                package="mower_base",
                ...
                parameters=[
                    {
                        ...
                        "virtual_obstacle_center_y": 0.6,
                        "virtual_obstacle_radius_m": 0.5,
                        "virtual_obstacle2_center_x": 7.0,
                        "virtual_obstacle2_center_y": -0.6,
                        "virtual_obstacle2_radius_m": 0.5,
                        "virtual_obstacle3_center_x": 10.0,
                        "virtual_obstacle3_center_y": -0.6,
